...
export default function ShapChart({ shapFeatures }) {
  ...
  return (
    <Paper withBorder p="xl" radius="md" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, minHeight: 220 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ top: 0, right: 24, left: 16, bottom: 0 }}>
            ...
            <XAxis type="number" domain={[0, maxVal * 1.15]} tickFormatter={v => v.toFixed(2)} tick={{ fontSize: 11, fill: '#4a5578' }} axisLine={{ stroke: 'rgba(255,255,255,0.07)' }} tickLine={{ stroke: 'rgba(255,255,255,0.07)' }} />
            <YAxis type="category" dataKey="name" width={170} tick={{ fontSize: 11, fill: '#6b7a99' }} axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
            <Bar dataKey="value" radius={[0, 4, 4, 0]}>
              {data.map((entry, i) => (
                <Cell key={i} fill={i === data.length - 1 ? '#6366f1' : i === data.length - 2 ? '#818cf8' : '#94a3b8'} fillOpacity={0.85} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <Text size="xs" c="dimmed" mt="sm" ta="center">
        SHAP TreeExplainer · LightGBM Regressor · 8,791 listings from ikman.lk
      </Text>
    </Paper>
  )
}
